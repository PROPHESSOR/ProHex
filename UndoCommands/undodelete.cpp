...
UndoDelete::UndoDelete(DataStorage *dataStorage, uint8_t data, int64_t position) {
    ...
    m_data      = data;
    m_position  = position;
    setText(QObject::tr("Restore 0x%1 at %2").arg(QString::number(m_data, 16)).arg(m_position));
}
...
void UndoDelete::redo() {
    ...
    setText(QObject::tr("Remove 0x%1 at %2").arg(QString::number(m_data, 16)).arg(m_position));
}
